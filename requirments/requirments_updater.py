...
def add_to_requirements(dependencies, req_file_path=None):
    ...
    req_dir = os.path.dirname(req_file_path) if (
        os.path.dirname(req_file_path)
        ) else os.getcwd()
    ...
    if os.path.exists(req_file_path):
        with open(req_file_path, 'r') as f:
            existing_deps = {line.strip() for line in f if line.strip() and (
                not line.startswith('#'))}

    new_deps = set(dependencies) - existing_deps
    if not new_deps:
        print("All specified dependencies are already in requirements.txt or "
              "no new dependencies provided.")
        return
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python add_to_requirements.py "
              "<dependency1> [dependency2 ...] [path/to/requirements.txt]")
        sys.exit(1)
